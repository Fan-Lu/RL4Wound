...
dataFrame.to_csv('~/Desktop/Close_Loop_Actuation/Wound_2.csv', index=False)
...
u_app_ch7[0]= aaa_ch7
u_app_ch8[0] = aaa_ch8
df = pd.read_csv('~/Desktop/Close_Loop_Actuation/Output/Wound_2.csv')
...
for i in range(1,Nnc):
    ...
    m_size = len(df.index)
    time.sleep(4.5)
    row_index = -1  # normally it is -1 to read the last row for testing on file that doesnt change, set the value to i
    if row_index < 0:
        while m_size == m_size_old:
    ...
    # field names
    csv_table = {'t(s)': [time_table], 'V1(v)': [v1_table], 'V2(v)': [v2_table], 'V3(v)': [v3_table],'V4(v)': [v4_table],'V5(v)': [v5_table], 'V6(v)': [v6_table], 'V7(v)': [v7_table], 'V8(v)': [v8_table]}
    print(dataFrame)
    dataFrame = pd.DataFrame(csv_table)
    dataFrame.to_csv('~/Desktop/Close_Loop_Actuation/Wound_2.csv', mode='a', index=False, header=False)
